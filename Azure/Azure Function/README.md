...
## Steps to perform OCR on entire PDF document in Azure Functions
...
2. Select the framework to Azure Functions and select HTTP triggers as follows.
...
3. Install the [Syncfusion.PDF.OCR.NET](https://www.nuget.org/packages/Syncfusion.PDF.OCR.NET) NuGet package as a reference to your .NET Core application [NuGet.org](https://www.nuget.org/).
...
6. Include the following namespaces in [Function1.cs](Perform_OCR_Azure_Functions/Function1.cs) file to perform OCR for a PDF document using C#.
...
7. Add the following code snippet in the Function1 class to perform OCR for a PDF document in Azure Functions.
...
8. Now, check the OCR creation in the local machine.

### Steps to publish as Azure Functions 

Step 1: Right-click the project and select Publish. Then, create a new profile in the Publish Window. So, you can create the Azure Function App with a consumption plan.
...
Step 3: Now, go to the Azure portal and select the Functions Apps. After running the service, click Get function URL > Copy. Include the URL as a query string in the URL. Then, paste it into the new browser tab. You will get the PDF document as follows.
